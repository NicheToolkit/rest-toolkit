...
abstract class BaseAbstractPipeline<E_IN, E_OUT, S extends BaseStream<E_OUT, S>>
        extends DefaultPipelineHelper<E_OUT> implements BaseStream<E_OUT, S> {
    private static final String MSG_STREAM_LINKED = "stream has already been operated upon or closed";
    private static final String MSG_CONSUMED = "source already consumed or closed";

    @SuppressWarnings("rawtypes")
    private final BaseAbstractPipeline sourceStage;

    @SuppressWarnings("rawtypes")
    private final BaseAbstractPipeline previousStage;

    protected final int sourceOrOpFlags;

    @SuppressWarnings("rawtypes")
    private BaseAbstractPipeline nextStage;
    ...
    BaseAbstractPipeline(Supplier<? extends Spliterator<?>> source,
    ...
    BaseAbstractPipeline(Spliterator<?> source,
                         int sourceFlags, boolean parallel) {
        this.previousStage = null;
    ...
    BaseAbstractPipeline(BaseAbstractPipeline<?, E_IN, ?> previousStage, int opFlags) {
    ...
    private Spliterator<?> sourceSpliterator(int terminalFlags) {
        ...
        if (isParallel() && sourceStage.sourceAnyStateful) {
            ...
            int depth = 1;
            for (@SuppressWarnings("rawtypes") BaseAbstractPipeline u = sourceStage, p = sourceStage.nextStage, e = this;
                 u != e;
                 u = p, p = p.nextStage) {
    ...
    final DefaultStreamShape getSourceShape() {
        @SuppressWarnings("rawtypes")
        BaseAbstractPipeline p = BaseAbstractPipeline.this;
        while (p.depth > 0) {
            p = p.previousStage;
        }
    ...
    final <P_IN> DefaultSink<P_IN> wrapSink(DefaultSink<E_OUT> sink) {
        ...
        for (@SuppressWarnings("rawtypes") BaseAbstractPipeline p = BaseAbstractPipeline.this; p.depth > 0; p=p.previousStage) {
