...
public abstract class RequestParsingArgumentResolver implements HandlerMethodArgumentResolver {
        ...
        if (GeneralUtils.isNotEmpty(declaredFields)) {
            for (Field field : declaredFields) {
                ...
                RestParsingFieldPack.RestParsingFieldPackBuilder fieldBuilder = RestParsingFieldPack.builder()
                        .declaringType(field.getDeclaringClass()).name(fieldName).fieldName(fieldName).field(field);
                ...
                if (parsingUnderline || GeneralUtils.isNotEmpty(AnnotationUtils.getAnnotation(field, JsonParsingUnderline.class))) {
                    ...
                    fieldBuilder.name(fieldName).fieldName(fieldName);
                ...
                if (GeneralUtils.isNotEmpty(parent)) {
                    ...
                    String nestedName = nestedPrefix + "." + fieldName;
                    fieldBuilder.name(nestedName);
                }
    ...
    private Map<String, Object> parsingParameterMap(List<RestParsingFieldPack> parsingFields, Map<String, String[]> parameterMap) {
        Map<String, RestParsingFieldPack> parsingFieldMap = parsingFields.stream().collect(Collectors.toMap(RestParsingFieldPack::getName, Function.identity(), (oldValue, newValue) -> newValue));
        Map<String, Object> parameters = new LinkedHashMap<>();
        Map<String, Map<String, Object>> nestedParameters = new LinkedHashMap<>();
        List<String> ignoredValues = parsingProperties.getIgnored().getValues();
        for (Map.Entry<String, String[]> entry : parameterMap.entrySet()) {
            String key = entry.getKey();
            String[] value = entry.getValue();
            RestParsingFieldPack fieldPack = parsingFieldMap.get(key);
            if (GeneralUtils.isEmpty(fieldPack) || GeneralUtils.isEmpty(value)) {
                continue;
            }
            String fieldName = fieldPack.getFieldName();
            if (fieldPack.isNestedField() && GeneralUtils.isNotEmpty(fieldPack.getParent())) {
                RestParsingFieldPack parent = fieldPack.getParent();
                String parentFieldName = parent.getFieldName();
                Map<String, Object> nested = nestedParameters.get(parentFieldName);
    ...
    private void parsingFieldValue(RestParsingFieldPack fieldPack,String fieldName, String[] value, Map<String, Object> parameters,List<String> ignoredValues) {
        if (fieldPack.isMultiple()) {
            ...
            parameters.put(fieldName, value[0]);
        }
    }
